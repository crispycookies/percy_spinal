...
import percytheperceptron.{predictor, predictor_delayed_train}
import spinal.sim._
...
object PredictorTests {
  def main(args: Array[String]): Unit = {
    ...
    val core3 = SimConfig.withWave.compile(new predictor(bit_width = 16, feature_count = 3, table_size = 2, address_bit_width = 16, index_bit_width = 16, 0,1,0,1))
    core3.doSim("Check Wiring"){ dut =>
      dut.clockDomain.forkStimulus(10)
      dut.io.address #= 0
      dut.io.taken #= 1
      println(dut.io.prediction.toInt)
    ...
    val core4 = SimConfig.withWave.compile(new predictor_delayed_train(bit_width = 16, feature_count = 3, table_size = 2, address_bit_width = 16, index_bit_width = 16, -1,1,0,1))
    core4.doSim("Check Wiring Delayed"){ dut =>
      dut.clockDomain.forkStimulus(10)
      dut.io.address #= 0
      dut.io.taken #= 1
      println(dut.io.prediction.toInt)
